...
func TestInvalidTable(t *testing.T) {
	...
	if isValidTable {
		t.Fatalf("check table err!")
	}
}

func TestAloneTable(t *testing.T) {
	table := [][]int{
		{2, 2, 2, 2, 0, 0, 0, 0, 0},
		{0, 0, 0, 0, 2, 0, 0, 0, 0},
		{0, 0, 0, 0, 2, 0, 0, 0, 0},
...
func TestEasyTable(t *testing.T) {
	table := [][]int{
		{2, 2, 1, 2, 0, 0, 0, 0, 0},
		{0, 0, 2, 2, 1, 2, 0, 0, 0},
		{0, 0, 2, 0, 2, 0, 0, 0, 0},
...
func TestHardTable(t *testing.T) {
	table := [][]int{
		{2, 2, 1, 2, 0, 0, 0, 0, 0},
		{0, 0, 2, 2, 1, 2, 0, 0, 0},
		{0, 0, 2, 0, 2, 0, 2, 0, 0},
...
func testTable(t *testing.T, table [][]int) {
	if len(table) <= 0 {
		t.Fatalf("empty table")
	}
	if err := Init(table, ChengYuMock, 1); err != nil {
		t.Fatalf("TestAloneTable err: %v", err)
	}
	result := make(map[string]bool, 0)
	selectedMap := make(map[string]bool, 0)
	RecursionGenerate(0, []string{}, result, selectedMap)
	if len(result) <= 0 {
		t.Fatalf("empty result!")
	} else {
		index := 0
		for item, _ := range result {
			index++
			if index > 5000 {
				break
			}
			...
			if !Check(strings.Split(item, ",")) {
				t.Fatalf("Check not pass!")
			}
			if index < 2 {
				tableXyWord, _ := TableXyWordSet(item)
				for _, val := range tableXyWord {
					fmt.Printf("%+v\n", val)
				}
			}
		}
